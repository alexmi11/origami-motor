...
const overlap = (p0, p1) =>
  0.00000001 > Math.abs(p0[0] - p1[0]) + Math.abs(p0[1] - p1[1]);
...
export function offset(polylines, delta, ops = {}) {
  ...
  */
  ...
  const toClipperFormat = (pl) => pl.map(([x, y]) => ({ X: x, Y: y }));

  const fromClipperFormat = (pl) => pl.map(({ X, Y }) => [X, Y]);
  ...
  final.forEach((pl, i) => {
    ...
    polylines[i].push([pl[0][0], pl[0][1]]);
  });
